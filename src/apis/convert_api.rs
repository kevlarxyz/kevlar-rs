...
/// struct for typed errors of method [`getaconversion`]
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]

pub enum GetaconversionError {
    UnknownValue(serde_json::Value),
}

pub async fn getaconversion(
    configuration: &configuration::Configuration,
    accept: &str,
    x_vault_request: &str,
    x_vault_token: &str,
) -> Result<String, Error<GetaconversionError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/convert", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetaconversionError> =
