...
/// struct for typed errors of method [`deployasmartcontract`]
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]

pub enum DeployasmartcontractError {
    UnknownValue(serde_json::Value),
}

pub async fn deployasmartcontract(
    configuration: &configuration::Configuration,
    account_name: &str,
    accept: &str,
) -> Result<String, Error<DeployasmartcontractError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str =
        format!("{}/accounts/{}/deploy", local_var_configuration.base_path, account_name);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<DeployasmartcontractError> =
