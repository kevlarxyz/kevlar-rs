...
/// struct for typed errors of method [`geterc20totalsupply`]
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]

pub enum Geterc20totalsupplyError {
    UnknownValue(serde_json::Value),
}

pub async fn geterc20totalsupply(
    configuration: &configuration::Configuration,
    account_name: &str,
    accept: &str,
) -> Result<String, Error<Geterc20totalsupplyError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!(
        "{}/accounts/{}/erc20/totalSupply",
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<Geterc20totalsupplyError> =
