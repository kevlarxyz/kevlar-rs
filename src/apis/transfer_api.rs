...
/// struct for typed errors of method [`transferanerc20`]
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]

pub enum Transferanerc20Error {
    UnknownValue(serde_json::Value),
}
...
pub async fn transfer_eth(
    ...
    x_vault_token: &str,
    body: Option<&str>,
) -> Result<String, Error<TransferEthError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str =
        format!("{}/accounts/{}/transfer", local_var_configuration.base_path, account_name);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<TransferEthError> =
...
pub async fn transferanerc20(
    configuration: &configuration::Configuration,
    account_name: &str,
    accept: &str,
) -> Result<String, Error<Transferanerc20Error>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str =
        format!("{}/accounts/{}/erc20/transfer", local_var_configuration.base_path, account_name);
    let mut local_var_req_builder =
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<Transferanerc20Error> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
